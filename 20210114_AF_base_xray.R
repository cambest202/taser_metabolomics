...
xray_resp <- inner_join(xray_erosion, resp_ints_A, by='Sample_Name')
xray_melt <- xray_resp[,c(1,12:1469)]
xray_melt <- melt(xray_melt)
xray_melt$DAS44 <- xray_melt$DAS44
xray_melt$Sharp_Score <- xray_resp$ΔSharp_Score
...
xray_pre_limma <- xray_resp[, c(4,1470,12:1469)]
mean(xray_pre_limma$ΔSharp_Score)
median(xray_pre_limma$ΔSharp_Score)
histogram(xray_pre_limma$ΔSharp_Score)

xray_pre_limma$XRay_Response[xray_pre_limma$ΔSharp_Score >=2] <- 'Negative'
xray_pre_limma$XRay_Response[xray_pre_limma$ΔSharp_Score < 2] <- 'Positive'
...
xray_limma$Sig_Peaks <- ifelse(xray_limma$P.Value<0.05 & xray_limma$identification != '', xray_limma$Peak_ID, '')
...
xray_limma_hmdb <- distinct(xray_limma_hmdb, Peak_ID, .keep_all = TRUE)
xray_limma_hmdb <- xray_limma_hmdb[-8,]
...
xray_resping <- xray_pre_limma_2[,-1]

set.seed(42)
index <- createDataPartition(xray_resping$XRay_Response, p = 0.7, list = FALSE)
train_data <- xray_resping[index, ]
test_data  <- xray_resping[-index, ]
...
write.csv(imps_hmdb_id, '20210114_AF_base_xray_FI.csv')
...
xray_melt_top <- subset(xray_melt, xray_melt$Peak_ID %in% imps_hmdb$Peak_ID)
...
xray_melt_top$Xray_Response <- 0
xray_melt_top$Xray_Response[xray_melt_top$Sharp_Score >=2] <- 'Negative'
xray_melt_top$Xray_Response[xray_melt_top$Sharp_Score < 2] <- 'Positive'

stat_test <- xray_melt_top %>%
  group_by(Peak_ID) %>%
  rstatix::wilcox_test(Peak_Intensity ~ Xray_Response) %>%
  adjust_pvalue(method = "BH") %>%
  add_significance()

write.csv(stat_test, '20210114_AF_base_xray_stats.csv')
