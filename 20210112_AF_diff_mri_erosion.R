...
mri_resp$MRI_Response <- 0
mri_pre_limma <- mri_resp[, c(2,1475,17:1474)]
mean(mri_pre_limma$ΔMRI_Erosion)
median(mri_pre_limma$ΔMRI_Erosion)
histogram(mri_pre_limma$ΔMRI_Erosion)

mri_pre_limma$MRI_Response[mri_pre_limma$ΔMRI_Erosion >=2] <- 'Negative'
mri_pre_limma$MRI_Response[mri_pre_limma$ΔMRI_Erosion <2] <- 'Positive'
mri_pre_limma_2 <- mri_pre_limma

mri_pre_limma %>%
  ggplot(aes(length(MRI_Response),
             fill=MRI_Response))+
  geom_histogram()+
  facet_wrap(~MRI_Response)+
  theme(legend.position = 'none')

mri_pre_limma <- mri_pre_limma[,-1]
rownames(mri_pre_limma) <- paste0(rownames(mri_pre_limma), mri_pre_limma$MRI_Response)
mri_pre_limma <- mri_pre_limma[,-1]
...
mri_limma$Sig <- 0
mri_limma$Sig <- ifelse(mri_limma$P.Value <0.05, 1, 0) 
...
ggplot(data=mri_limma_hmdb, aes(x=logFC, y=-log10(P.Value), 
  ...
  labs (x='LogFC',
        y='-Log p-value',
        title='MRI Erosion: Differential Analysis of Metabolites \nBetween Erosion Scoring Outcomes') +
...
summary(test_data$MRI_Response)
test_data$MRI_Response <- as.factor(test_data$MRI_Response)
confusionMatrix(test_results, test_data$MRI_Response)

final <- data.frame(actual = test_data$MRI_Response,
...
write.csv(imps_hmdb_id, '20210112_AF_mri_erosion_FI.csv')
